...
| ACP SDK            | `@agentclientprotocol/sdk@0.26.x` | 已含 experimental elicitation；升级到 1.x 必须单独进行 |
| Pi                 | README 声明 `>=0.80.4`            | 需要在 `C0.2` 改成“实际测试过的窗口”                   |
| Node               | adapter 声明 `>=20`               | 新 Pi 版本可能要求更高 Node；E2E 单独建矩阵            |
| existing tests     | 95/95 通过                        | 以 fake RPC/unit 为主，不能证明真实插件兼容            |
...
- [ ] 记录 adapter、Pi、ACP SDK、Node、Zed 和另一客户端的精确版本/SHA。
- [ ] 将当前开放式 `Pi >=0.80.4` 改成“测试窗口 + 超出窗口 best effort”。
- [ ] Pi 和 ACP SDK 不在同一 PR 中同时升级。
